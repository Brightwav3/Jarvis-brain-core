...
export class ComponentRegistry {
  ...
  register(component: RuntimeComponent): void {
    ...
    this.#entries.push({ component, record: { id: component.id, name: component.name, version: component.version, required: component.required, state: 'registered' } });
    void this.events.publish({ type: 'component.registered', source: component.id, timestamp: new Date().toISOString() });
  }
  async startAll(): Promise<void> {
    for (const entry of this.#entries) try { await entry.component.start(); entry.record.state = 'running'; } catch (error) { entry.record.state = 'failed'; entry.record.reason = error instanceof Error ? error.message : String(error); await this.events.publish({ type: 'component.failed', source: entry.component.id, timestamp: new Date().toISOString(), error: entry.record.reason }); if (entry.component.required) throw error; }
  }
  ...
  list(): ComponentRecord[] { return this.#entries.map(({ record }) => ({ ...record })); }
}
