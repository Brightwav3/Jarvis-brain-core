...
export class HealthService {
  constructor(private readonly components: () => ComponentRecord[] | Promise<ComponentRecord[]>) {}
  async status(): Promise<HealthReport> {
    const subsystems = (await this.components()).map((component) => component.state === 'failed'
