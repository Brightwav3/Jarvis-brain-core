...
type Dependencies = { identity: string; version: string; status(): unknown; components(): ComponentRecord[] | Promise<ComponentRecord[]> };
export class LocalApiServer {
  ...
  async start(port: number, host = '127.0.0.1'): Promise<{ port: number }> {
    ...
    this.#server = createServer((request, response) => { void (async () => {
      ...
      const data = request.url === '/health' ? await health.status() : request.url === '/status' ? this.dependencies.status() : request.url === '/components' ? await this.dependencies.components() : request.url === '/version' ? { identity: this.dependencies.identity, version: this.dependencies.version } : undefined;
